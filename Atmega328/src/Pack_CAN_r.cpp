...
_Medicion Pack_CAN_r::Get_Medicion(uint8_t *data,uint8_t lengt){
    _tag_Data trama;
    uint8_t *_pack;

    _pack= &trama._init;
    for(int i =0; i<lengt;i++) *_pack++ =*data++; //Copia datos de puntero externo en puntero a trama de datos

    return trama._data; // Retorna la longitud del vector
}
